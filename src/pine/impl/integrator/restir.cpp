...
RestirIntegrator::RestirIntegrator(Accel accel, Sampler sampler, LightSampler light_sampler,
  ...
  if (max_path_length <= 0)
    SEVERE("`RestirIntegrator` expect `max_path_length` to be positive, get", max_path_length);
}
struct RestirIntegrator::Vertex {
  ...
  static Vertex first_vertex(vec2i q) { return Vertex(q, 0, {}, 0.0f, true); }
  vec2i q;
  int length;
  Interaction it;
  float pdf;
...
struct ReSample {
  float target_pdf() const { return average(Lo); }
...
void RestirIntegrator::render(Scene& scene) {
  ...
  for (int si = 0; si < spp; si++) {
    parallel_for(film.size(), [&](vec2i p) {
      ...
      if (p.x % 64 == 0) set_progress(progress_2d(p, film.size()) / spp + float(si) / spp);
    });
    parallel_for(film.size(), [&](vec2i q) {
      ...
      for (int i = 0; i < 16; i++) {
        ...
        if (dot(initial_samples[q].nv, initial_samples[qn].nv) < 0.995f) continue;
        if (psl::abs(initial_samples[q].t - initial_samples[qn].t) > 0.1f) continue;
...
RestirIntegrator::RadianceResult RestirIntegrator::radiance(Scene& scene, Ray ray, Sampler& sampler,
  ...
  //     if (!pv.is_delta) result.light_pdf = scene.env_light->pdf(pv.it, ray.d);
  //   }
  //   return result;
  // }
  ...
  //   if (!pv.is_delta) result.light_pdf = light_sampler.pdf(pv.it, *it, ray);
  //   return result;
  // }
  ...
  // if (with_probability(1 - rr_m, sampler)) return result;
  ...
  //   if (pv.length == 0) initial_samples[pv.q].pdf = bs->pdf;
