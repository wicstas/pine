...
MltIntegrator::MltIntegrator(Accel accel, int avg_spp, LightSampler light_sampler,
  ...
  if (max_path_length <= 0)
    Fatal("`MltIntegrator` expect `max_path_length` to be positive, get", max_path_length);
}
struct MltIntegrator::Vertex {
  ...
  static Vertex first(int max_path_length) {
    return Vertex(max_path_length, 0, 0, 0.0f, true);
  }
  int max_path_length;
  int length;
  int diffuse_length;
...
void MltIntegrator::render(Scene& scene) {
  ...
  for (int i = 0; i < n_threads(); i++)
    startup_samplers.push_back(BlueSobolSampler(startup_spp));
  auto I = Atomic<float>();
  ...
  parallel_for(n_total, [&](int i) {
    ...
    if (lp > 1e-6f)
      film.add_radiance(pfilmp, Lp / lp * (1 - a));
    if (l > 1e-6f)
      film.add_radiance(pfilm, L / l * a);
    if (sampler.randf() < a) {
      m_sampler.accept();
      pfilmp = pfilm;
      Lp = L;
    ...
    if (i % 100 == 0)
      set_progress(float(i) / n_total / 2);
  });

  film.scale(Int / avg_spp);

  for (int i = 0; i < n_threads(); i++)
    startup_samplers[i] = BlueSobolSampler(avg_spp);
  parallel_for(film.size(), [&](vec2i p) {
    Sampler& sampler = startup_samplers[threadIdx].start_pixel(p, 0);
    auto L = vec3(0.0f);
    ...
    if (p.x % 64 == 0)
      set_progress(0.5f + 0.5f * progress_2d(p, film.size()));
  });
}
