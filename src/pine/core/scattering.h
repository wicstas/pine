...
inline float Sin2Theta(vec3 w) {
    return psl::max(1.0f - Cos2Theta(w), 0.0f);
}
...
inline bool Refract(vec3 wi, vec3 n, float eta, vec3& wt, float* etap = nullptr) {
    ...
    float sin2ThetaI = psl::max(0.0f, 1.0f - psl::sqr(cosThetaI));
    float sin2ThetaT = sin2ThetaI / psl::sqr(eta);
    if (sin2ThetaT >= 1)
        return false;
...
inline float FrDielectric(float cosThetaI, float eta) {
    cosThetaI = psl::clamp(cosThetaI, -1.0f, 1.0f);
    if (cosThetaI < 0) {
        eta = 1 / eta;
        cosThetaI = -cosThetaI;
    }

    float sin2ThetaI = 1.0f - psl::sqr(cosThetaI);
    float sin2ThetaT = sin2ThetaI / psl::sqr(eta);
    if (sin2ThetaT >= 1.0f)
        return 1.0f;
    float cosThetaT = psl::sqrt(1.0f - sin2ThetaT);
    ...
    return (psl::sqr(rParl) + psl::sqr(rPerp)) / 2.0f;
}
...
struct TrowbridgeReitzDistribution {
  public:
    ...
    float D(vec3 wm) const {
        ...
        float cos4Theta = psl::sqr(Cos2Theta(wm));
        if (cos4Theta < 1e-6f)
            return 0.0f;
    ...
    float Lambda(vec3 w) const {
        ...
        float alpha2 = psl::sqr(CosPhi(w) * alphaX) + psl::sqr(SinPhi(w) * alphaY);
    ...
    vec3 SampleWm(vec3 w, vec2 u) const {
        ...
        float h = psl::sqrt(1.0f - psl::sqr(p.x));
