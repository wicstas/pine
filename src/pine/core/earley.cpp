...
// bool is_char(char c) { return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'); }
// bool is_number(char c) { return c >= '0' && c <= '9'; }

// psl::vector<EarleyNode> tokenize(psl::string input) {
//   auto tokens = psl::vector<EarleyNode>();

//   auto p = size_t(0);
//   auto parsing_number = false;
//   auto number_is_float = false;

//   for (size_t i = 0; i < input.size(); i++) {
//     auto c = input[i];
//     if (is_char(c)) {
//       if (parsing_number && !number_is_float) {
//         tokens.emplace_back("int", psl::vector_of(EarleyNode(input.substr(p, i - p), {})));
//         p = i;
//       }
//       continue;
//     } else if (is_number(c)) {
//       parsing_number = true;
//       continue;
//     } else if (parsing_number && c == '.') {
//       number_is_float = true;
//       continue;
//     }

//     if (parsing_number)
//       tokens.emplace_back(number_is_float ? "float" : "int",
//                           psl::vector_of(EarleyNode(input.substr(p, i - p), {})));
//     else
//       tokens.emplace_back(psl::to_string(c), psl::vector<EarleyNode>());
//     p = i;
//   }
...
//     bool no_prev() const { return prev_loc == invalid_pre_loc; }
...
//   };
//   psl::string sym(const State& state) { return grammar.rules[state.rule_index].sym; };
...
//         for (const auto& child : node.children) elements.push_back(build(build, child));
//         auto vector = elements[0].make_empty_vector();
//         for (auto& elem : elements) elem.move_append(vector);
//         return vector;
//       }
...
//     if (!model->use_string)
//       for (const auto& child : node.children) args.push_back(build(build, child));
...
//       for (const auto& arg : args) argsTypesName += arg.type_name() + psl::string(" ");
//       if (model->use_string)
