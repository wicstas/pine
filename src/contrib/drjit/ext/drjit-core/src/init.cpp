...
static_assert(sizeof(VariableKey) == 9 * sizeof(uint32_t),
              "VariableKey: incorrect size, likely an issue with padding/packing!");

static_assert(sizeof(tsl::detail_robin_hash::bucket_entry<VariableMap::value_type, false>) == 64,
...
#endif
      jitc_fail("jit_init(): creation of directory \"%s\" failed: %s", temp_path, strerror(errno));
  }
...
void jitc_shutdown(int light) {
  ...
  for (ThreadState *ts : state.tss) {
    ...
    if (!ts->mask_stack.empty())
      jitc_log(Warn, "jit_shutdown(): leaked %zu active masks!", ts->mask_stack.size());
  }
  ...
  if (!state.kernel_cache.empty()) {
    jitc_log(Info, "jit_shutdown(): releasing %zu kernel%s ..", state.kernel_cache.size(),
  ...
  if (!state.tss.empty()) {
    jitc_log(Info, "jit_shutdown(): releasing %zu thread state%s ..", state.tss.size(),
             state.tss.size() > 1 ? "s" : "");
    ...
    for (ThreadState *ts : state.tss) {
      ...
      if (!ts->prefix_stack.empty()) {
        for (char *s : ts->prefix_stack)
          free(s);
        jitc_log(Warn, "jit_shutdown(): leaked %zu prefix stack entries.", ts->prefix_stack.size());
        free(ts->prefix);
      }
    ...
    if (state.variables.empty() && !state.lvn_map.empty()) {
      for (auto &kv : state.lvn_map)
        jitc_log(Warn,
                 ...
                 "%u, %u, %u]",
                 kv.second, kv.first.size, type_name[kv.first.type], kv.first.dep[0],
                 kv.first.dep[1], kv.first.dep[2], kv.first.dep[3]);

      jitc_log(Warn,
               "jit_shutdown(): detected a common subexpression "
  ...
  if (std::max(state.log_level_stderr, state.log_level_callback) >= LogLevel::Warn) {
    ...
    for (auto &var : state.variables) {
      ...
      if (n_leaked < 10)
        jitc_log(
            ...
            "stmt=\"%s\", dep=[%u, %u, %u, %u])",
            var.first, (uint32_t)var.second.ref_count, (uint32_t)var.second.ref_count_se,
            type_name[var.second.type], var.second.size,
            var.second.is_literal() ? "<value>" : (var.second.stmt ? var.second.stmt : "<null>"),
            var.second.dep[0], var.second.dep[1], var.second.dep[2], var.second.dep[3]);
      else if (n_leaked == 10)
    ...
    if (state.variables.empty() && !state.extra.empty()) {
      jitc_log(Warn,
               "jit_shutdown(): %zu 'extra' records were not cleaned up:", state.extra.size());
      n_leaked = 0;
  ...
  if (backend == JitBackend::CUDA) {
    if ((state.backends & (uint32_t)JitBackend::CUDA) == 0) {
      ...
      if (jitc_cuda_cuinit_result == CUDA_ERROR_NOT_INITIALIZED) {
        ...
        jitc_raise(
            "jit_cuda_init(): the CUDA backend is not available "
    ...
    if ((state.backends & (uint32_t)JitBackend::LLVM) == 0) {
      ...
      jitc_raise(
          "jit_init_thread_state(): the LLVM backend is inactive "
...
void jitc_cuda_set_device(int device_id) {
  ...
  if ((size_t)device_id >= state.devices.size())
    jitc_raise("jit_cuda_set_device(%i): must be in the range 0..%i!", device_id,
               (int)state.devices.size() - 1);
...
void jitc_prefix_push(JitBackend backend, const char *label) {
  if (strchr(label, '\n') || strchr(label, '/'))
    jitc_raise(
        "jit_prefix_push(): invalid string (may not contain newline "
        "or '/' characters)");
...
void *jitc_find_library(const char *fname, const char *glob_pat, const char *env_var) {
#if !defined(_WIN32)
  ...
  if (!handle) {
    if (env_var_val) {
      jitc_log(Warn, "jit_find_library(): Unable to load \"%s\": %s!", env_var_val, dlerror());
      return nullptr;
    }
    ...
    if (glob(glob_pat, GLOB_BRACE, nullptr, &g) == 0) {
      ...
      if (g.gl_pathc > 1) {
        jitc_log(Info,
                 "jit_find_library(): Multiple versions of "
...
KernelHistoryEntry *KernelHistory::get() {
  ...
  for (size_t i = 0; i < m_size; i++) {
    ...
    if (k.backend == JitBackend::CUDA) {
      cuEventElapsedTime(&k.execution_time, (CUevent)k.event_start, (CUevent)k.event_end);
