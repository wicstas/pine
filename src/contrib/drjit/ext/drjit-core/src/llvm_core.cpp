...
bool jitc_llvm_init() {
  ...
  if (!jitc_llvm_api_has_core()) {
    jitc_log(Warn,
             "jit_llvm_init(): detected LLVM version lacks core API "
  ...
  if (!jitc_llvm_api_has_pb_new() && !jitc_llvm_api_has_pb_legacy()) {
    jitc_log(Warn,
             "jit_llvm_init(): detected LLVM version lacks pass "
  ...
  jitc_llvm_disasm_ctx = LLVMCreateDisasm(jitc_llvm_target_triple, nullptr, 0, nullptr, nullptr);

  if (jitc_llvm_disasm_ctx) {
    if (LLVMSetDisasmOptions(jitc_llvm_disasm_ctx, LLVMDisassembler_Option_PrintImmHex |
  ...
  if (!strstr(jitc_llvm_target_features, "+fma")) {
    jitc_log(Warn,
             "jit_llvm_init(): your CPU does not support the `fma` "
  ...
  if (jitc_llvm_api_has_orcv2() && jitc_llvm_orcv2_init()) {
    ...
    jitc_log(Warn,
             "jit_llvm_init(): ORCv2/MCJIT could not be initialized, "
  ...
  jitc_log(Info,
           ...
           major_str, minor_str, patch_str, jitc_llvm_use_orcv2 ? "ORCv2" : "MCJIT",
           jitc_llvm_target_triple, jitc_llvm_target_cpu,
           jitc_llvm_opaque_pointers ? "opaque" : "typed", jitc_llvm_vector_width);
...
void jitc_llvm_update_strings() {
  ...
  jitc_llvm_ones_str = (char **)malloc(sizeof(char *) * (uint32_t)VarType::Count);
  ...
  for (uint32_t i = 0; i < (uint32_t)VarType::Count; ++i) {
    ...
    for (uint32_t j = 0; j < width; ++j) {
      ...
      if (vt == VarType::Bool)
        buf.put('1');
      else if (vt == VarType::Float16 || vt == VarType::Float32 || vt == VarType::Float64) {
        buf.put("0x");
...
void jitc_llvm_set_target(const char *target_cpu, const char *target_features,
...
void jitc_llvm_disasm(const Kernel &kernel) {
  ...
  for (uint32_t i = 0; i < kernel.llvm.n_reloc; ++i) {
    uint8_t *func_base = (uint8_t *)kernel.llvm.reloc[i], *ptr = func_base;
    if (i == 1)
      continue;
    char ins_buf[256];
    bool last_nop = false;
    ...
    do {
      size_t offset = ptr - (uint8_t *)kernel.data, func_offset = ptr - func_base;
      if (offset >= kernel.size)
        break;
      size_t size = LLVMDisasmInstruction(jitc_llvm_disasm_ctx, ptr, kernel.size - offset,
...
void jitc_llvm_compile(Kernel &kernel) {
  ...
  LLVMMemoryBufferRef llvm_buf =
      LLVMCreateMemoryBufferWithMemoryRange(buffer.get(), buffer.size(), kernel_name, 0);
  ...
  if (unlikely(error))
    jitc_fail(
        "jit_llvm_compile(): parsing failed. Please see the LLVM "
  ...
  if (unlikely(status))
    jitc_fail(
        "jit_llvm_compile(): module could not be verified! Please "
...
#define DRJIT_RUN_NEW_PASS_MANAGER()                                                        \
  ...
  LLVMErrorRef error_ref = LLVMRunPasses(llvm_module, "default<O2>", jitc_llvm_tm, pb_opt); \
  if (error_ref)                                                                            \
  ...
  std::vector<uint8_t *> reloc(callable_count_unique ? (callable_count_unique + 2) : 1);
  ...
  if (ptr == MAP_FAILED)
    jitc_fail("jit_llvm_compile(): could not mmap() memory: %s", strerror(errno));
#else
  void *ptr =
      VirtualAlloc(nullptr, jitc_llvm_memmgr_offset, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
  if (!ptr)
