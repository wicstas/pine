...
#if defined(_MSC_VER)
#if defined(_M_ARM64)
  return _InterlockedCompareExchange128_rel((__int64 volatile *)&ptr, (__int64)desired.value,
                                            (__int64)desired.task, (__int64 *)&expected);
#else
  return _InterlockedCompareExchange128((__int64 volatile *)&ptr, (__int64)desired.value,
                                        (__int64)desired.task, (__int64 *)&expected);
#endif
#else
  return __atomic_compare_exchange(&ptr, &expected, &desired, true, __ATOMIC_RELEASE,
                                   __ATOMIC_RELAXED);
#endif
}
...
TaskQueue::~TaskQueue() {
  uint32_t created = tasks_created.load(), deleted = 0, incomplete = 0, incomplete_size = 0;
  ...
  if (incomplete > 0)
    fprintf(stderr,
            "nanothread: %u tasks with %u work units were not "
            "completed!\n",
...
Task *TaskQueue::alloc(uint32_t size) {
  ...
  task->next = Task::Ptr();
  task->refcount.store(size + (size == 0 ? high_bit : (3 * high_bit)), std::memory_order_relaxed);
...
void TaskQueue::release(Task *task, bool high) {
  ...
  uint32_t ref_lo = (uint32_t)result, ref_hi = (uint32_t)(result >> 32);

  NT_ASSERT((!high || ref_hi > 0) && (high || ref_lo > 0));
  ref_hi -= (uint32_t)high;
  ref_lo -= (uint32_t)!high;

  NT_TRACE("dec_ref(%p, (%i, %i)) -> ref = (%u, %u)", task, (int)high, (int)!high, ref_hi, ref_lo);
  ...
  if (!high && ref_lo == 0) {
    ...
    for (Task *child : task->children) {
      ...
      NT_TRACE("notifying child %p of task %p: wait=%u", child, task, wait - 1);
      ...
      if (task->exception_used.load()) {
        ...
        if (child->exception_used.compare_exchange_strong(expected, true)) {
          NT_TRACE("propagating exception to child %p of task %p.", child, task);
          child->exception = task->exception;
        } else {
          NT_TRACE(
              "not propagating exception to child %p of "
              "task %p (already stored).",
              child, task);
        }
      }

      if (wait == 1) {
        NT_TRACE("Child %p of task %p is ready for execution.", child, task);
        push(child);
      }
    }
...
void TaskQueue::add_dependency(Task *parent, Task *child) {
  ...
  uint64_t refcount = parent->refcount.load(std::memory_order_relaxed);
  ...
  while (true) {
    if ((uint32_t)refcount == 0) {
      ...
      if (parent->exception_used.load()) {
        bool expected = false;
        if (child->exception_used.compare_exchange_strong(expected, true)) {
          NT_TRACE("propagating exception to child %p of task %p.", child, parent);
    ...
    if (parent->refcount.compare_exchange_weak(refcount, refcount + 1, std::memory_order_release,
  ...
  NT_TRACE("registering dependency: parent=%p, child=%p, child->wait=%u", parent, child, wait);
...
std::pair<Task *, uint32_t> TaskQueue::pop_or_sleep(bool (*stopping_criterion)(void *),
  ...
  while (true) {
    ...
    if (may_sleep && attempts >= NANOTHREAD_MAX_ATTEMPTS) {
      ...
      NT_TRACE(
          "pop_or_sleep(): falling asleep after %.2f milliseconds, "
          "sleep_state := (%u, %u)!",
          time_milliseconds() - start, (uint32_t)(value >> 32), (uint32_t)value);
      ...
      */
      ...
      NT_TRACE("pop_or_sleep(): woke up -- sleep_state=(%u, %u)", (uint32_t)(value >> 32),
               (uint32_t)value);
    }
  }
