...
inline bool iswordmiddle(char c) { return iswordstart(c) || isdigit(c); }
...
class string : public vector<char, string_allocator<char>> {
 public:
  ...
  string substr(Iterator p0, Iterator p1) const { return substr(p0 - begin(), p1 - p0); }
  string substr(Iterator p0) const { return substr(p0 - begin(), end() - p0); }
  class string_view subview(Iterator p0, Iterator p1) const;
  class string_view subview(Iterator p0) const;
