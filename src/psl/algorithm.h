...
auto range(It first, It last) {
  struct Wrapper {
    ...
    size_t size() const { return psl::distance(first, last); }
...
void move(OutputIterator auto d_first, Range auto&& input) {
  auto first = psl::begin(input);
  auto last = psl::end(input);
  for (; first != last; ++first, ++d_first) *d_first = psl::move(*first);
}

void copy_inplace(OutputIterator auto d_first, Range auto&& input) {
  auto first = psl::begin(input);
  auto last = psl::end(input);
  for (; first != last; ++first, ++d_first) psl::construct_at(&(*d_first), *first);
}

void move_inplace(OutputIterator auto d_first, Range auto&& input) {
  auto first = psl::begin(input);
  auto last = psl::end(input);
  for (; first != last; ++first, ++d_first) psl::construct_at(&(*d_first), psl::move(*first));
}
...
void fill_inplace(Range auto&& output, const auto& value) {
  auto first = psl::begin(output);
  auto last = psl::end(output);
  for (; first != last; ++first) psl::construct_at(&*first, value);
}
...
struct less<void> : StrictOrdering {
  ...
  bool operator()(const auto& l, const auto& r) const { return l < r; }
};
...
template <typename T>
struct _less_than {
  bool operator()(const auto& x) const { return x < value; }
  T value;
};
template <typename T>
struct _equal_to {
...
auto find(Range auto&& range, const auto& value) { return find_if(range, equal_to(value)); }
...
size_t count(Range auto&& range, const auto& x) { return count_if(range, equal_to(x)); }
...
namespace pipe {
auto operator|(Range auto&& range, auto&& adapter) { return adapter(range); }
}  // namespace pipe
...
auto transform(ARange&& range, F f) {
  struct Ranger {
    ...
    Iterator begin() { return {f, psl::begin(range)}; }
    Iterator end() { return {f, psl::end(range)}; }
    size_t size() { return psl::size(range); }
...
auto filter(ARange&& range, F f) {
  struct Ranger {
    ...
    Iterator end() { return {f, psl::end(range), psl::end(range)}; }
...
auto reverse_adapter(ARange&& range) {
  struct Ranger {
    struct Iterator {
      decltype(auto) operator*() const { return *rit(); }
      Iterator& operator++() {
        ++it;
        return *this;
      }
      ...
      auto operator->() { return &(*rit()); }
      bool operator==(const Iterator& b) const { return it == b.it; }
      bool operator!=(const Iterator& b) const { return !(*this == b); }

      IteratorTypeT<ARange> rit() const { return begin + (end - it - 1); }

      IteratorTypeT<ARange> it;
      IteratorTypeT<ARange> begin;
      IteratorTypeT<ARange> end;
    };
    Iterator begin() { return {psl::begin(range), psl::begin(range), psl::end(range)}; }
    Iterator end() { return {psl::end(range), psl::begin(range), psl::end(range)}; }
    size_t size() { return psl::size(range); }
    ARange range;
  };
...
auto indirection(ARange&& range) {
  struct Ranger {
    ...
    Iterator begin() { return {psl::begin(range)}; }
    Iterator end() { return {psl::end(range)}; }
    size_t size() { return psl::size(range); }
    ARange range;
  };
  return Ranger{psl::forward<ARange>(range)};
...
auto trim(Range auto&& range, size_t offset, size_t len) {
  return psl::range(psl::next(psl::begin(range), psl::min(offset, psl::size(range))),
                    psl::next(psl::begin(range), psl::min(offset + len, psl::size(range))));
}
...
auto variance(Range auto&& range) {
  ...
  for (auto&& x : range) var += (x - mean) * (x - mean);
...
auto tie_adapter(RangeA&& a, RangeB&& b) {
  struct Ranger {
    struct Iterator {
      ...
      bool operator==(const Iterator& b) const { return a_it == b.a_it || b_it == b.b_it; }
      bool operator!=(const Iterator& b) const { return !(*this == b); }

      IteratorTypeT<RangeA> a_it;
      IteratorTypeT<RangeB> b_it;
    };
    Iterator begin() { return {psl::begin(a), psl::begin(b)}; }
    Iterator end() { return {psl::end(a), psl::end(b)}; }
