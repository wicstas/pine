...
void main() {
    ...
    // float rad = (atan(Dr / depth) / Pi) * mr_x;
    // for(int x = max(0, int(px - rad)); x < min(mr_x, px + rad); x++) {
    //     for(int y = max(0, int(py - rad)); y < min(mr_y, py + rad); y++) { 
    //         if (depth < color[x][y].w)
    //             color[x][y] = vec4(Dc, depth);
    //     }
    // }
    }
    ...
    integral /= mr_x * mr_y;
    out_color = vec4(direct + cd * integral, 1);
